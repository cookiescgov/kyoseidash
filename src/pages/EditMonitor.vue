        ...
        <div>
            ...
            <form @submit.prevent="submit">
                <div class="shadow-box shadow-box-with-fixed-bottom-bar">
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="my-3">
                                ...
                                >
                                    ...
                                    <optgroup label="Kyosei Dash">
                                        <option value="prtg">PRTG Sensor</option>
                                    </optgroup>
                                </select>
                                <i18n-t
                            ...
                            <!-- Kyosei Dash — PRTG Monitor Type -->
                            <div v-if="monitor.type === 'prtg'" class="my-3">
                                <label for="prtg-server" class="form-label">PRTG Server</label>
                                <select id="prtg-server" v-model.number="monitor.prtg_server_id" class="form-select" required>
                                    <option :value="null" disabled>— {{ $t("Select") }} —</option>
                                    <option v-for="s in prtgServerList" :key="s.id" :value="s.id">
                                        {{ s.name }} ({{ s.url }})
                                    </option>
                                </select>
                                <div class="form-text">
                                    <router-link to="/settings/prtg">{{ $t("Manage PRTG Servers") }}</router-link>
                                </div>
                            </div>
                            <div v-if="monitor.type === 'prtg'" class="my-3">
                                <label for="prtg-sensor-id" class="form-label">Sensor ID</label>
                                <input id="prtg-sensor-id" v-model.number="monitor.prtg_sensor_id" type="number" class="form-control" min="1" required />
                                <div class="form-text">Find this in PRTG — sensor URL `id=...` or use the importer (Phase 4).</div>
                            </div>
                            <!-- Kyosei Dash — universal Device field (Phase 5 hierarchy key) -->
                            <div class="my-3">
                                <label for="prtg-device" class="form-label">{{ $t("Device") }} ({{ $t("optional") }})</label>
                                <input id="prtg-device" v-model="monitor.prtg_device" type="text" class="form-control" placeholder="e.g. 10.1.1.16 — groups monitors in Devices view" />
                            </div>
                    ...
                    <!-- Kyosei Dash: Connection Launchers -->
                    <div class="shadow-box big-padding text-center mb-4">
                        <h2 class="mb-4">{{ $t("Connection Launchers") }}</h2>
                        <p class="text-start text-muted small mb-3">
                            {{ $t("Add click-to-connect buttons (RDP, SSH, VNC, Telnet, Web). Shown on the monitor detail page.") }}
                        </p>
                        <div v-for="(c, i) in monitor.connections" :key="i" class="row g-2 align-items-end mb-2 text-start">
                            <div class="col-md-2">
                                <label class="form-label small mb-1">{{ $t("Type") }}</label>
                                <select v-model="c.type" class="form-select form-select-sm">
                            ...
                            <div v-if="c.type !== 'web' && c.type !== 'custom'" class="col-md-3">
                                <label class="form-label small mb-1">{{ $t("Host") }}</label>
                                <input v-model="c.host" type="text" class="form-control form-control-sm" placeholder="10.1.1.16" />
                            </div>
                            <div v-if="c.type !== 'web' && c.type !== 'custom'" class="col-md-1">
                                <label class="form-label small mb-1">{{ $t("Port") }}</label>
                                <input v-model.number="c.port" type="number" class="form-control form-control-sm" />
                            </div>
                            <div v-if="c.type === 'ssh'" class="col-md-2">
                                <label class="form-label small mb-1">{{ $t("User") }}</label>
                                <input v-model="c.user" type="text" class="form-control form-control-sm" placeholder="root" />
                            </div>
                            <div v-if="c.type === 'web' || c.type === 'custom'" class="col-md-5">
                                <label class="form-label small mb-1">URL</label>
                                <input v-model="c.url" type="text" class="form-control form-control-sm" placeholder="https://..." />
                            </div>
                            <div class="col-md">
                                <label class="form-label small mb-1">{{ $t("Label") }}</label>
                                <input v-model="c.label" type="text" class="form-control form-control-sm" :placeholder="c.type.toUpperCase()" />
                            </div>
                            <div class="col-md-auto">
                                <button type="button" class="btn btn-outline-danger btn-sm" @click="monitor.connections.splice(i, 1)">
                                    <font-awesome-icon icon="times" />
                                </button>
                            </div>
                        </div>
                        <button type="button" class="btn btn-outline-primary btn-sm mt-2"
                            @click="monitor.connections.push({ type: 'rdp', host: '', port: 3389, label: '' })">
                            <font-awesome-icon icon="plus" /> {{ $t("Add Connection") }}
                        </button>
                    </div>
                ...
                `
{
    ...
    methods: {
        /**
         * Kyosei Dash — load PRTG server list for the type dropdown
         * @returns {void}
         */
        loadPrtgServerList() {
            this.$root.getSocket().emit("getPrtgServerList", (res) => {
                if (res && res.ok) {
                    this.prtgServerList = res.list || [];
                }
            });
        },
        /**
