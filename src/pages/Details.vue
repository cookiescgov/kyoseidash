<template>
    <transition name="slide-fade" appear>
        <div v-if="monitor">
            ...
            <div v-if="monitor.connections && monitor.connections.length" class="connections-row mt-2 mb-2">
                <a
                    v-for="(c, i) in monitor.connections"
                    :key="i"
                    :href="buildConnectionUrl(c)"
                    :target="c.type === 'web' ? '_blank' : '_self'"
                    rel="noopener noreferrer"
                    class="btn btn-sm btn-outline-primary me-2 mb-1"
                >
                    <font-awesome-icon :icon="connectionIcon(c.type)" />
                    {{ c.label || c.type.toUpperCase() }}
                </a>
            </div>
            <p class="url">
                <a
...
const ChannelsChart = defineAsyncComponent(() => import("../components/ChannelsChart.vue"));
...
export default {
    ...
    methods: {
        getResBaseURL,
        /**
         * Kyosei Dash — build a launch URI for a connection entry
         * @param {object} c connection
         * @returns {string} URI
         */
        buildConnectionUrl(c) {
            switch (c.type) {
                case "rdp": return `rdp://${c.host}${c.port ? ":" + c.port : ""}`;
                case "ssh": return `ssh://${c.user ? c.user + "@" : ""}${c.host}${c.port ? ":" + c.port : ""}`;
                case "vnc": return `vnc://${c.host}${c.port ? ":" + c.port : ""}`;
                case "telnet": return `telnet://${c.host}${c.port ? ":" + c.port : ""}`;
                case "web": return c.url || "#";
                case "custom": return c.url || "#";
                default: return "#";
            }
        },
        /**
         * Kyosei Dash — pick a FontAwesome icon for a connection type
