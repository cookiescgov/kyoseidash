...
const { prtgServerSocketHandler } = require("./socket-handlers/prtg-server-socket-handler");
const { channelsSocketHandler } = require("./socket-handlers/channels-socket-handler");
const { prtgImportSocketHandler } = require("./socket-handlers/prtg-import-socket-handler");
    ...
    io.on("connection", async (socket) => {
        ...
        socket.on("add", async (monitor, callback) => {
            try {
                ...
                monitor.connections = JSON.stringify(Array.isArray(monitor.connections) ? monitor.connections : []);
        ...
        socket.on("editMonitor", async (monitor, callback) => {
            try {
                ...
                bean.connections = JSON.stringify(Array.isArray(monitor.connections) ? monitor.connections : []);
                bean.prtg_server_id = monitor.prtg_server_id || null;
                bean.prtg_sensor_id = monitor.prtg_sensor_id || null;
                bean.prtg_device = monitor.prtg_device || null;
        ...
        prtgServerSocketHandler(socket);
        channelsSocketHandler(socket);
        prtgImportSocketHandler(socket);
